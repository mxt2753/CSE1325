...
public class MainWin extends JFrame {
    ...
    public MainWin(String title) {
       ...
       // Add a New ELSA store icon
        JButton anewButton  = new JButton(new ImageIcon("gui/resources/new.png"));
    ...
    protected void onInsertOrderClick() {
        // Implement the onInsertOrderClick() method as described in the instructions
    }
    */
    ...
    protected void onInsertOrderClick() 
    {
        ...
        ImageIcon imageIcon = new ImageIcon("gui/resources/add_order.png");
    ...
    protected  String[] UnifiedDialog(String[] fields, String title, String iconFilename)
    {
        ...
        Object[] dialogObjects = new Object[fields.length * 2];
        for (int i = 0; i < fields.length; i++) {
            dialogObjects[i * 2] = new JLabel(fields[i]);
            dialogObjects[i * 2 + 1] = new JTextField();
        }
        int result = JOptionPane.showConfirmDialog(this, dialogObjects, title, JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, icon);
        if (result == JOptionPane.OK_OPTION) {
            String[] output = new String[fields.length];
            for (int i = 0; i < fields.length; i++) {
                output[i] = ((JTextField) dialogObjects[i * 2 + 1]).getText();
            }
            return output;
        }
        return null;
